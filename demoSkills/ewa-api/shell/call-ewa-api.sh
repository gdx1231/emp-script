...
resolve_password_file() {
    ...
    local password_file="${API_PASSWORD#file://}"
    password_file="${password_file/#\~/$HOME}"

    if [[ ! -r "$password_file" ]]; then
        echo "[ERROR] 无法读取密码文件: $password_file" >&2
        exit 1
    fi
...
方法:
  ...
  previewBusinessXml <db> <table> <ftype> <optype> <xmlname> [output] [scriptpath]
                                  预览业务 XML（不保存）
  createBusinessXml <db> <table> <ftype> <optype> <xmlname> <itemname> [admid] [scriptpath]
                                  生成并保存业务 XML
  showScriptPaths                 列出可用配置存储路径
  getXmlFile <xmlname> [output] [scriptpath]
                                  直接读取 XML 文件内容
...
do_update_conf_item() {
    ...
    if [[ "$xml" == @* ]]; then
        local xml_file="${xml#@}"
        if [[ ! -r "$xml_file" ]]; then
            log_error "无法读取 XML 文件: $xml_file"
            exit 1
        fi
        xml="$(<"$xml_file")"
    fi
...
do_preview_business_xml() {
    ...
    local params="method=previewBusinessXml&db=${db}&tablename=${tablename}&frametype=${frametype}&operationtype=${operationtype}&xmlname=${xmlname}"
    [ -n "$output" ] && params="${params}&output=${output}"
    [ -n "$scriptpath" ] && params="${params}&scriptpath=${scriptpath}"

    log_info "预览业务 XML: ${tablename} ${frametype} ${operationtype} -> ${xmlname}"
    send_request "$params"
}
...
do_create_business_xml() {
    ...
    log_info "创建业务 XML: ${tablename} ${frametype} ${operationtype} -> ${xmlname} / ${itemname}"
    send_request "$params"
}
...
do_get_xml_file() {
    ...
    local params="method=getXmlFile&xmlname=${xmlname}"
    [ -n "$output" ] && params="${params}&output=${output}"
    [ -n "$scriptpath" ] && params="${params}&scriptpath=${scriptpath}"

    log_info "读取 XML 文件: $xmlname"
    send_request "$params"
}
