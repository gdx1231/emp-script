...
This document collects the system table definitions (DDL) required by emp-script (EWA):

- **EWA configuration tables** — store XML configurations (MySQL / HSQLDB / SQL Server)
- **RESTful API tables** — RESTful interface definitions (MySQL / HSQLDB)
- **Module import/export tables** — module management (MySQL / HSQLDB)

---

## Convert File-based Config to JDBC Config

Convert XML file-based configuration into database (JDBC) configuration:
...
## EWA Configuration Tables (MySQL)
...
## EWA Configuration Tables (HSQLDB)
...
> For a complete HSQLDB initialization script, see the built-in `conf.help.hsqldb` under `src/main/resources`.

## EWA Configuration Tables (SQL Server)
...
## Module Import/Export Tables (MySQL)
...
## Module Import/Export Tables (HSQLDB)
