...
本文档汇总 emp-script (EWA) 运行所需的系统表结构定义，包括：

- **EWA 配置表** — 存储 XML 配置（MySQL / HSQLDB / SQL Server）
- **RESTful API 表** — RESTful 接口定义（MySQL / HSQLDB）
- **Module 导入/导出表** — 模块管理（MySQL / HSQLDB）

---

## 转换文件配置到数据库配置

将 XML 文件配置转换为数据库（JDBC）配置：
...
> 完整的 HSQLDB 初始化脚本可参考内置的 `conf.help.hsqldb`（见 `src/main/resources`）。
