...
//		String path = u.getName();
	...
	private String ewaHelpDocuments(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			if (confs.isJdbc()) {
				// 从数据库获取所有配置
				this.addJdbcEndpoints(endpoints);
			} else {
				// 从 XML 配置获取
				this.addEwaConfEndpoints(endpoints);
			}
	...
	/**
	 * 从数据库获取所有 RESTful 配置并添加到文档中
	 */
	private void addJdbcEndpoints(JSONArray endpoints) {
		try {
			// 获取所有目录
			...
			for (int i = 0; i < tbCatalog.getCount(); i++) {
				...
				String pathFull = catalogRow.getCell("cat_path_full").toString();
				...
				for (int j = 0; j < tbMethods.getCount(); j++) {
					...
					JSONObject endpoint = this.createEndpointDescription(methodRow, catalogRow);
	...
	/**
	 * 从 XML 配置获取所有 RESTful 配置并添加到文档中
	 */
	private void addEwaConfEndpoints(JSONArray endpoints) {
		try {
			// 使用反射或其他方式获取所有配置
			// 这里暂时留空，需要实现
	 ...
	 * 创建单个端点的描述
	 */
	private JSONObject createEndpointDescription(DTRow methodRow, DTRow catalogRow) {
		...
		if (!xmlName.isEmpty() && !itemName.isEmpty()) {
			this.addXItemFields(endpoint, xmlName, itemName);
		}
	 ...
	 */
	private void addXItemFields(JSONObject endpoint, String xmlName, String itemName) {
		try {
